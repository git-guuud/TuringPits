...
import { MOCK_PROVIDER_META } from "@turingpits/players/dist/envelope.js";
...
export async function buildProvider(): Promise<ProviderBundle> {
  ...
  if (computeKey && providerAddress) {
    ...
    return { provider, isMock: false, teeSigner };
  }
  ...
  return { provider: mock, isMock: true, teeSigner: mock.signerAddress };
}

/** Provider metadata registered at createMatch — must match what the provider signs with. */
