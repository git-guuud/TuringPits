 ...
 */
import type { ConnStatus, MatchFeed, WsMessage } from "../lib/types.js";
...
export function createWsFeed(opts: WsFeedOptions): MatchFeed {
  ...
  const setStatus = (s: ConnStatus) => statusListeners.forEach((fn) => fn(s));

  function connect() {
    if (closed) return;
    // First attempt is "connecting"; any later attempt means we dropped → "reconnecting".
