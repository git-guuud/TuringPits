...
const isTransient = (m) => /429|fetch failed|EAI_AGAIN|ECONNRESET|ETIMEDOUT|socket hang up|502|503|504/i.test(m);
    ...
    async complete(prompt) {
      ...
      for (;;) {
        ...
        try {
          ...
          if (isTransient(msg) && attempts++ < 10) {
            const backoff = msg.includes("429") ? 30_000 : 8_000;
            console.log(`  …transient (${msg.slice(0, 48)}), retry ${attempts} in ${backoff / 1000}s`);
            await sleep(backoff);
            continue;
          }
          throw err;
...
const onTurn = (turn, state) => {
  ...
  log(`> ${turn.speech.trim()}`);
  log(`- action: \`${d.action}\` → seat ${d.target} (${nameOf(d.target)})  ·  attested \`${turn.attestation.source}\` ${ok}  ·  alive: [${alive.join(", ")}]\n`);
};

let match;
try {
  match = await players.playMatch({ seed: SEED, n: N, nonce: NONCE, personas: PERSONAS, players: playerSeats, onTurn });
} catch (err) {
...
const m = players.MOCK_PROVIDER_META; // provider metadata the live envelope carries
